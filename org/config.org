...
** Browse Kill Ring
   ...
   Stop doing C-y M-y M-y M-y. Do C-c y instead.
...
** window-number mode
   ...
   Some of the things below are commented out so we can rely on
   ace-window-jump instead, but I still need window-number for some of
   the views below.
...
** bars, menus and numbers
   ...
   horizontal-scroll-bar-mode doesn't exist in emacs before 24.4, so
   we have to wrap it with a (when (boundp...)) to keep from calling
...
** Powerline
   ...
   Because why not steal good things from vim.
...
** password store
   ...
   Password store seems pretty good. I need longer passwords though.
...
** sunshine
   ...
   What is the weather like at home?

   #+BEGIN_SRC emacs-lisp
     (setq sunshine-location "London, England")
...
*** ido vertical mode
    ...
    Because sometimes you just can't see what is going on
    horizontally.
...
**** magit and github pull requests

     Github diffs just aren't that good. I'd like to be able to review
     in emacs.
...
*** git-gutter-mode+
    ...
    #+BEGIN_SRC emacs-lisp
      (global-set-key (kbd "s-f") 'git-gutter+-next-hunk)
      (global-set-key (kbd "s-b") 'git-gutter+-previous-hunk)
    #+END_SRC
...
*** ace-window-jump
    ...
    I'm mostly putting this in so that I use ace-jump more for in
    buffer navigating.
...
*** org-headlines
    ...
    #+BEGIN_SRC emacs-lisp
      (add-hook 'org-mode-hook
                (lambda () (local-set-key (kbd "s-h") 'helm-org-in-buffer-headings)))
    #+END_SRC
...
***** Make sure you are clocked in when doing something

      #+BEGIN_SRC emacs-lisp
        (require 'clocker)
        (diminish 'clocker-mode "⌚")
        (add-hook 'kill-emacs-hook
                  (lambda () (clocker-mode 0)))
      #+END_SRC
...
**** Go to the current clock

     I want to be able to go to the current clock to update things and
     take notes.
...
*** git-auto-commit
    ...
    I want to keep all of this lovely stuff under version control but
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (eval-after-load 'git-auto-commit
        '(progn
           (setq gac-dir-set
                 ;; we don't push our org files
                 '(("~/org")))))

      (setq gac-default-cmd-git-push "/bin/true")
...
*** Spell Checking
    ...
    #+BEGIN_SRC emacs-lisp
      (setq erc-autojoin-channels-alist '(("freenode.net" "##cleanweb" "#kixi" "#theodi" "#ldnclj" "#clojure-emacs" "#clojurescript" "#nhsengland" "#clojure-beginners")))
    #+END_SRC
...
*** Switch to irc
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (global-set-key [s-f11] 'switch-to-irc)
    #+END_SRC
...
*** Handle idling
    ...
    Mark me away if I'm idle for more than 5 mins
...
*** Colour the whole message by nick

    A bit more angry fruit salad, but handy when trying to track
    people in a channel.
     ...
     (defun comms ()
       ...
       (if (< (frame-width) 275)
           (progn
             ...
             (switch-to-buffer "#ldnclj")
             (window-number-select 5)
             (switch-to-buffer "#clojurescript")
...
***** install
      ...
      #+BEGIN_SRC emacs-lisp
        (require 'cider)
        (setq cider-mode-line '(:eval (format " Cλ[%s]" (cider-current-ns))))
      #+END_SRC
...
*** emms keybindings
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (global-set-key (kbd "M-<f9>") 'emms-play-dired)
...
*** A bit of white noise
    ...
    Sometimes I just need a thunderstorm to keep out the noise. I
    find them restful.
